...
static void addBuiltinButtons() {
    ...
    // The renderer lays the first button nearest the right edge.
    g_pGlobalState->buttons.push_back(
        SHyprButton{.userfg = true, .fgcol = CHyprColor(0xfff4f4f5), .bgcol = CHyprColor(0x00000000), .size = 30, .icon = "×", .action = eTouchbarButtonAction::CLOSE});
    g_pGlobalState->buttons.push_back(
        SHyprButton{.userfg = true, .fgcol = CHyprColor(0xfff4f4f5), .bgcol = CHyprColor(0x00000000), .size = 30, .icon = "□", .action = eTouchbarButtonAction::MAXIMIZE});
    g_pGlobalState->buttons.push_back(
        SHyprButton{.userfg = true, .fgcol = CHyprColor(0xfff4f4f5), .bgcol = CHyprColor(0x00000000), .size = 30, .icon = "−", .action = eTouchbarButtonAction::MINIMIZE});
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    ADD_CONFIG(iconOnHover, CBoolValue, "icon_on_hover", "Only show custom button icons while hovered", false);
    ...
    return {"hyprtouchbar", "Modern, touch-friendly and CSD-aware titlebars", "hyprtouchbar contributors", "1.2.1"};
}
